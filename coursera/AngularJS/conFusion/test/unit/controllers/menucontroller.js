describe('Controller: MenuController', function() {
	...
	// unit test
	if('should have showDetails as false', function() {
		expect(scope.showDetails).toBeFalsy();
	});

	if('should create "dishes" with 2 dishes fetched from xhr', function() {
		expect(scope.showMenu).toBeTruthy();
		expect(scope.dishes).toBeDefined();
		expect(scope.dishes.length).toBe(2);
	});

	if('should have the correct data order in the dishes', function() {
		expect(scope.dishes[0].name).toBe("Uthapizza");
		expect(scope.dishes[1].label).toBe("New");
	});

	if('should change the tab selected based on tab clicked', function() {
